...
interface Props {
  ...
  /** Open the in-site source reader for a claim (verify chip on ungrounded claims). */
...
export default function BundleView(props: Props) {
  const { data, onSelect, onHover, onOpenSource, selectedId, hoveredId, highlightedIds } = props;
...
function ClaimRow({
  ...
  // Ungrounded: no verbatim quote, or the engine flagged it for verification.
  const ungrounded = !claim.quote || !!claim.verify;
  ...
  return (
    <div
      ...
      onMouseLeave={() => onHover(null)}
      className={`transition ${inset ? "" : "rounded-sm border"}`}
      style={{
    ...
    >
      ...
      {/* Make absence visible: an ungrounded claim gets a verify call-to-action
          straight to the source, rather than a silent gap. */}
      {ungrounded && (
        <div className={`flex items-center gap-1.5 ${inset ? "px-3 pb-2.5" : "px-3 pb-3"}`}>
          <VerifyChip
            reason={claim.verify_reason ?? "No verbatim quote anchored to this claim"}
            onClick={onOpenSource ? () => onOpenSource(claim.id) : undefined}
          />
        </div>
      )}
    </div>
